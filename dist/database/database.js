...
const path_1 = __importDefault(require("path")); // Import path for directory handling
const config_1 = __importDefault(require("../config")); // Import the default config object
...
class Database extends events_1.EventEmitter {
    ...
    constructor(options) {
        super();
        this.timer = (0, profiling_1.createTimer)(); // General purpose timer
        this.isClosed = false;
        this.isReady = false;
        // Background Tasks
        this.autoSaveTimer = null;
        // Concurrency Control
        this.activeSearchPromises = new Set();
        ...
        this.options = {
            vectorSize: options.vectorSize ?? config_1.default.defaults.vectorSize,
            clustering: { ...config_1.default.clustering, ...options.clustering },
            partitioning: {
                ...config_1.default.partitioning,
                ...options.partitioning,
            },
            indexing: { ...config_1.default.indexing, ...options.indexing },
            cacheSize: options.cacheSize ?? config_1.default.defaults.cacheSize,
            maxConcurrentSearches: options.maxConcurrentSearches ?? config_1.default.defaults.maxConcurrentSearches,
            persistence: { ...config_1.default.persistence, ...options.persistence },
            backup: { ...config_1.default.backup, ...options.backup }, // Keep for config, but logic removed
            monitoring: { ...config_1.default.monitoring, ...options.monitoring },
        };
        ...
        this.monitor = this.options.monitoring.enable
            ? new vector_monitoring_1.VectorDBMonitor({
    ...
    async _initialize() {
        try {
            ...
            const baseDir = path_1.default.join(process.cwd(), this.options.persistence.dbPath || 'database'); // Default path
            ...
            this.vectorDB = new partitioned_vector_db_1.PartitionedVectorDB({
                // Pass necessary options from this.options
                partitionsDir: path_1.default.join(baseDir, "partitions"),
            ...
            this.unifiedSearch = new unified_search_1.UnifiedSearch(this.vectorDB, {
    ...
    _ensureDirectoryExists(dirPath, purpose) {
        try {
    ...
    _setupEventListeners() {
        if (!this.vectorDB || typeof this.vectorDB.on !== 'function')
            return;
        const dbEmitter = this.vectorDB;
        // Generic handler to forward events, clear cache, and update monitor
        const handleDbEvent = (eventName, data) => {
            if (this.isClosed)
                return;
            ...
            const shouldUpdateMetrics = ['vector:add', 'vector:delete', 'vectors:bulkAdd', 'partition:loaded', 'partition:unloaded'].includes(eventName);
        ...
        this.unifiedSearch.on('search:complete', (data) => {
            if (this.monitor) {
                ...
                if (this.metrics.queryTimes.length > 100)
                    this.metrics.queryTimes.shift(); // Keep last 100 times
            }
    ...
    async _updateMonitorDbMetrics() {
        if (!this.monitor || !this.options.monitoring.enableDatabaseMetrics)
            return;
        // Prevent updates if DB isn't ready or is closing
        if (!this.vectorDB || !this.isReady || this.isClosed)
            return;
        try {
    ...
    async _assertReady(operation = 'Operation') {
        console.log('[Database] Asserting readiness for operation:', operation);
        if (this.isClosed)
            throw new Error(`Database is closed. Cannot perform ${operation}.`);
        if (!this.isReady) {
            console.log(`[Database] Waiting for database readiness before performing ${operation}...`);
            await this.initializationPromise;
            // Re-check after waiting
            if (this.isClosed)
                throw new Error(`Database was closed during initialization wait. Cannot perform ${operation}.`);
            if (!this.isReady)
                throw new Error(`Database initialization failed. Cannot perform ${operation}.`);
        }
    }
    /**
    ...
    async updateMetadata(id, metadataUpdate) {
     ...
     * ```
     */
    async getMetadataWithField(criteria, values, option) {
     ...
     */
    async findNearest(query, k, options = {}) {
    ...
    getVectorDB() {
        if (!this.isReady && !this.isClosed)
            console.warn('Accessing VectorDB instance before Database is fully ready.');
        if (this.isClosed)
            throw new Error('Cannot access VectorDB: Database is closed.');
        return this.vectorDB;
    }
    /** Gets the UnifiedSearch instance. */
    getUnifiedSearch() {
        if (!this.isReady && !this.isClosed)
            console.warn('Accessing UnifiedSearch instance before Database is fully ready.');
        if (this.isClosed)
            throw new Error('Cannot access UnifiedSearch: Database is closed.');
        return this.unifiedSearch;
    }
    /** Gets the total count of vectors across all configured partitions. */
    async getTotalVectorCount() {
