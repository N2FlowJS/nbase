...
exports.ProductQuantization = void 0;
...
class ProductQuantization {
    constructor(options = {}) {
        ...
        this.dimensionModels = new Map();
        // Maps vector IDs to their PQ codes for each dimension
        this.codes = new Map();
        this.dimensionCodes = new Map();
        // Track vector dimensions
        this.vectorDimensions = new Map();
        this.defaultVectorSize = options.vectorSize || config_1.default.defaults.vectorSize || 1024;
     ...
     * @private
     */
    _calculateOptimalSubvectorParams(vectorDimension) {
     ...
     * @private
     */
    _validateAndAdjustSubvectorConfig() {
     ...
     * @private
     */
    _calculateSubvectorParams(dimension) {
    ...
    async train(vectors, dimensionOrOptions, maybeOptions) {
        ...
        if (dimension !== undefined && this.dynamicDimensions) {
            ...
            const kmeans = new kmeans_1.KMeans(this.numClusters, 100, 0.001);
            try {
     ...
     */
    static async loadModel(json, options = {}) {
        let data;
        // Check if json is a file path
        if (json.endsWith('.json')) {
            try {
                const fileContent = await fs_1.promises.readFile(json, 'utf8');
        ...
        if (data.trained && data.centroids) {
            pq.trained = true;
            pq.centroids = data.centroids.map((subq) => subq.map((centroid) => new Float32Array(centroid)));
        }
            ...
            for (const [dimStr, modelData] of Object.entries(data.dimensionModels)) {
                ...
                if (model.trained && model.centroids) {
                    pq.dimensionModels.set(dim, {
                        numSubvectors: model.numSubvectors,
                        centroids: model.centroids.map((subq) => subq.map((centroid) => new Float32Array(centroid))),
                        trained: true,
                    });
                }
            }
     ...
     */
    distanceWithTables(tables, code) {
        if (!this.trained) {
            throw new Error('Product quantizer not trained');
        }
...
//# sourceMappingURL=product_quantization.js.map