...
class ProductQuantization {
    ...
    private dimensionModels: Map<number, {
    ...
    async train(
        vectors: Vector[],
        dimensionOrOptions?: number | TrainingOptions,
        maybeOptions?: TrainingOptions
    ): Promise<void> {
    ...
    serialize(): string {
        ...
        if (this.trained) {
            data.centroids = this.centroids.map(subq => 
                subq.map(centroid => Array.from(centroid))
            );
        }
        ...
        for (const [dim, model] of this.dimensionModels.entries()) {
            if (model.trained) {
                data.dimensionModels[dim] = {
                    ...
                    centroids: model.centroids.map(subq => 
                        subq.map(centroid => Array.from(centroid))
                    ),
                    trained: true
                };
    ...
    static async loadModel(json: string, options: LoadModelOptions = {}): Promise<ProductQuantization> {
        ...
        if (data.trained && data.centroids) {
            pq.trained = true;
            pq.centroids = data.centroids.map((subq: number[][]) => 
                subq.map(centroid => new Float32Array(centroid))
            );
        }
            ...
            for (const [dimStr, modelData] of Object.entries(data.dimensionModels)) {
                ...
                if (model.trained && model.centroids) {
                    pq.dimensionModels.set(dim, {
                        numSubvectors: model.numSubvectors,
                        centroids: model.centroids.map((subq: number[][]) => 
                            subq.map(centroid => new Float32Array(centroid))
                        ),
                        trained: true
                    });
     ...
     */
    setCodes(
        codes: Map<string | number, Uint8Array> | Record<string, Uint8Array>,
        dimension?: number
    ): void {
        // Convert to Map if needed
        const codeMap = codes instanceof Map ? codes : new Map(
        ...
        return {
            ...
            memory: {
                ...
                totalCodes: this.codes.size + 
                    Array.from(this.dimensionCodes.values())
    ...
    getMemoryUsage(): number {
        ...
        if (this.trained) {
            ...
            centroidSize += this.centroids.reduce((sum, subquantizer) => 
                sum + subquantizer.reduce((subSum, centroid) => 
                    subSum + centroid.length * 4, 0), 0);
        }
        
        // Add size of dimension-specific models
        for (const [dimension, model] of this.dimensionModels.entries()) {
            if (model.trained) {
                centroidSize += model.centroids.reduce((sum, subquantizer) => 
                    sum + subquantizer.reduce((subSum, centroid) => 
                        subSum + centroid.length * 4, 0), 0);
            }
        }
        ...
        for (const [dimension, codeMap] of this.dimensionCodes.entries()) {
            const numSubvectors = this.dimensionModels.has(dimension) ? 
                this.dimensionModels.get(dimension)!.numSubvectors : 
                Math.ceil(dimension / this.subvectorSize);
...
export default ProductQuantization;
